...
AlphaFairLoadController::AlphaFairLoadController(
...
StarHubPlanMessage::BackoffConfig AlphaFairLoadController::generate(
...
double AlphaFairLoadController::clampProbability(double value) const
{
	return std::max(_config.minProbability, std::min(value, _config.maxProbability));
}

double AlphaFairLoadController::clampStep(double delta) const
{
	return std::max(-_config.maxProbabilityStep, std::min(delta, _config.maxProbabilityStep));
}

double AlphaFairLoadController::utilityGradient(double pTx, double n) const
{
	const double p = clampProbability(pTx);
	const double q = std::max(_config.epsilon, 1.0 - p);

	const double S = std::max(_config.epsilon, n * p * std::pow(q, n - 1.0));
	const double dS = n * std::pow(q, n - 2.0) * (1.0 - n * p);

	return dS / std::pow(S, _config.alpha);
}
